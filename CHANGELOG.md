...
- **Two `tomo lcm` compacts running at once no longer clobber each other's session rewrite.** The rewrite staged into a single fixed `<session>.compacting.tmp`, so a second invocation — an agent-triggered compact and a hand-run one, or two housekeeping turns over the same file — wrote into the very file the first was about to rename into place, publishing the other process's bytes over the session (in the test that reproduces it, the whole conversation is replaced by one line). Staging now uses the pid+uuid temp name `prune-tools.ts` and `writeFileAtomicSync` already use, and unlinks its own corpse if the write or rename throws instead of leaving it beside the transcript. The archive append also moved to *after* the rename commits: done before, a compact that then failed to publish left the transcript holding events that were still in the session file, and the retry archived them a second time.
