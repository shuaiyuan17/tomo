...
| `showThinking` | boolean | Optional. Default `false`. Deliver the model's `thinking` content blocks to the chat alongside its reply, each prefixed with `💭`. Purely a content-block-type decision — outbound text is never pattern-matched. When the flag is OFF the SDK runs thinking `display: "omitted"`, which leaves thinking blocks empty; a thinking block that still has text in it under `omitted` is treated as a message the model put in the wrong block type and is delivered as if it were a `text` block (no `💭` prefix) — decided from block type and length only, never from what the text says. Also settable with `TOMO_SHOW_THINKING`. **Read at session start only.** The flag also sets the SDK's thinking `display` (`omitted` when off, `summarized` when on), and that is fixed when the session spawns — changing it mid-session takes effect on the next session, so run `tomo restart` to apply it now. |
