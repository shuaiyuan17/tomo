...
- **A malformed channel entry no longer prints the Telegram bot token.** `configIssues` described an invalid value with a bare `JSON.stringify`, and `parseChannels` validates a channel entry as one object — so a single hand-edit mistake in a sibling field (`"allowlist": "123456789"` instead of an array, the easy one) stringified the whole entry, `token` included. That string is rendered in three places: `tomo status`, the `tomo config` banner, and `assertConfigValid()`'s throw, which `start.ts` `console.error`s — under launchd that appends the token to `~/.tomo/logs/launchd.err.log`, a file that goes into bug reports. `litellm.apiKey` had the same path, and `groupSecret` leaked whenever it was the value that failed validation. Secret-named fields are now reduced to `***` plus their last four characters before the value is stringified, which keeps the message actionable — the label names the entry, the zod path names the failing field, and the operator can still tell *which* token is on disk — without publishing the credential. Non-secret siblings are printed verbatim, because seeing what you actually typed is the point of the message.

  `src/logger.ts` had no pino `redact` config at all, so there was no second line of defence: any `log.info({ channel }, …)` would have written a token to `~/.tomo/logs/tomo.log` in full. It now redacts the same field names at up to two levels of nesting (pino's `censor` default, `[Redacted]`). What counts as a secret lives in one place, `src/redact.ts`, shared by both surfaces: name contains `token`/`secret`/`password`/`apikey`/`authorization`/`cookie`/`credential`, or is a `…Key` with a credential qualifier (`apiKey`, `privateKey`, `clientKey`). Deliberately **not** matched: a bare `key`, and `sessionKey`/`storeKey`/`chatKey`. In this codebase those are routing identifiers — `key` is the session key in `agent.ts`'s elicitation logging — and blanking them would cost the diagnostics an operator reads the log for while protecting nothing, since no config field is named `key`.
