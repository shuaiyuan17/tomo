...
- **Fabricated inbound markers in outgoing text are flagged instead of trusted.** The model sometimes writes a line shaped like a fresh inbound message — `[imessage · Sat 08/29 08:25 PDT] …`, `[group "Family"] kw: …`, `<tomo-event type="cron" …>`, or the legacy `System:` / `[System: …]` prefixes — inside its own reply, and then answers it as though the owner had typed it. Nothing is injected mid-turn (owner, 2026-08-16: "from my side, it's you who sent it to me"), so the inbound envelope is the wrong place to look; the new `src/agent/inbound-markers.ts` looks at the outlet. `detectFabricatedMarkers(text)` reports every line that *starts* with one of those shapes and is not inside a code fence — the start-of-line rule is what keeps Tomo able to discuss its own plumbing, since a marker quoted in backticks or mid-sentence never begins a line, and the fence exception covers the other legitimate case, pasting a log or transcript excerpt (an unfenced `System: …` line is still flagged; a fenced one is not). Fences follow CommonMark on both ends — openers take at most three leading spaces (four, or a leading tab, is an indented code block) and a backtick fence's info string may not contain a backtick; closers need the same delimiter character, at least the opener's length, and nothing else on the line. That strictness is the point rather than pedantry: pasted material is exactly what contains stray delimiter lines, so a `~~~` line must not close a ``` fence and three backticks must not close a four-backtick one, or the rest of the excerpt is handed straight back to the detector. Lines are split on any break convention, CRLF included — a stray `\r` used to leave the closing rule unsatisfiable and silently disable the exception for every Windows-pasted log. Leading characters that render as nothing (BOM, zero-width space and joiners, bidi marks, soft hyphen, word joiner, non-breaking and ideographic spaces) count as indentation rather than as content, so a marker hidden behind one is still flagged. **Policy is mark, don't truncate**: unlike a training-scaffold leak (`scaffold-filter.ts`, which cuts), the prose around a fabricated marker is usually a real reply, so the block ships whole with one advisory line, `FABRICATED_MARKER_NOTICE`, prepended. Each hit logs a warning with the session key, the shape and the offending line (clipped to 120 chars) and publishes a `fabricated-marker` watch event, which the Prometheus exporter turns into `tomo_fabricated_markers_total{shape}`. The counter is **detection**-counted, not delivery-counted — it fires on the model's output, before anything knows whether the block will find a sink, be refused as silent/error text, or be dropped by a suppressed turn — because the question it answers is "how often does the model do this". The session key rides the log line and the watch event but is deliberately not a metric label: prom-client retains every label combination for the daemon's lifetime and session keys are open-ended, so a `session` label would grow the series set without bound.

  **The advisory reaches the channel and nothing else.** `TurnRequest.onBlock` now takes two arguments — the model's verbatim words, and the copy that goes on the wire — so the transcript (`recall_conversation`), the agent-error classifier, the silence policy and the value `send()` resolves with all keep the model's literal output. Only `sender.deliver` sees the marked text.

  **`send_message` direct mode is guarded the same way.** It was the one outlet that shipped model-authored text without the check: `channel.send` got the text as given. The same detector now runs on it — same shapes, same start-of-line and code-fence rules, same advisory prepended to the wire copy only (the plain send, or the caption of an attachment send) — and the transcript entry and the pending note keep the verbatim text, as the block path keeps its transcript. Each hit is logged and counted against the *target* session, the conversation the fabricated line would have misled, and the tool result carries a note naming the shape so the calling model learns what happened. Delegate mode needed nothing: it wakes a real turn whose blocks ship through the guarded path. The counter stays in-process (the Prometheus series and the watch feed); it is deliberately not persisted per session, so `/status` does not show it.

  The stamp and group-tag formatters move into the same module (`formatInboundStamp`, `formatGroupTag`) and are now the single source of truth for both halves: `injectTimestamp` and `Agent.formatGroupText` build markers with them, and the tests feed each formatter's own output back through the detector, so a change to either shape cannot silently blind the guard.

- **`schedule_enable` MCP tool.** Enables (or disables) a scheduled task by id without deleting and re-creating it. The agent could previously only create, list and remove jobs, so a job the daemon disabled — a one-shot whose run was interrupted by a restart, a job that hit the resume cap — had no path back except re-creating it from scratch, losing its id and history. Enabling clears the interrupted state as well as flipping the flag, so the next daemon start does not settle the job as interrupted and disable it all over again — but only when that state is genuinely **settled** (the status says interrupted *and* the run token is already acknowledged). A job whose run is still in flight is never adjudicated by a toggle: recovery leaves `lastStatus: "interrupted"` on a resumed recurring job and then dispatches it again, so a status-only check would have let a disable/enable during that resumed run acknowledge the live token and lose the interruption entirely — the next restart would re-fire it with no `[resumed]` marker. A resumed dispatch also clears the status as it starts, so nothing downstream reads a live run as settled. The tool description says what an interrupted job means and that its work may already be done, because "just re-enable it" is the wrong reflex for a task that may already have sent, ordered, or posted. The tool is **scoped by `canManageJob` (`src/cron/scope.ts`)**: a session may always manage its own jobs; a `dm:` caller is the owner and may additionally manage group-keyed jobs and jobs carrying no session key, but never another identity's DM jobs; a group session may manage only its own. Scoping purely on equality would have been wrong in both directions — a summoned group's background turns run on `dm:<owner>` while the jobs they manage carry the group key, so the owner would have been locked out of jobs they created for their own group, and jobs with no `sessionKey` (legacy records, jobs orphaned by a rewrite) would have been enableable by nobody. The refusal does not name the owning session. Scoping of `schedule_list` / `schedule_create` / `schedule_remove` is #319, which should adopt the same predicate.
...
- **The daemon installs `unhandledRejection` / `uncaughtException` handlers at start.** There were none: the only `process.on` calls were the two signal handlers, so an unhandled rejection anywhere — a fire-and-forget send, a background runner, a channel retry — terminated the process with none of `shutdown()`'s cleanup (stale pid file, orphaned `imsg rpc` child, metrics port and watch socket held until the OS reaped them), and under `KeepAlive` the replacement could then hit the stale-pid refusal. New `src/process-handlers.ts`, installed as the first thing after the synchronous pid-file claim (console-only, so a module-level throw in the config or agent imports dies with a greppable marker instead of Node's bare default output) and upgraded to pino once the logger exists — install is idempotent by replacement, so two installs never stack two listeners. The two events are treated differently on purpose. An **unhandled rejection is logged and survived**: it is almost always one orphaned promise while the rest of the daemon is intact, and the `log.error` already lands on the watch bus as an `issue` event and bumps `tomo_issues_total{level="error"}`. Because that fan-out reaches every connected `tomo watch` client, the path is rate-limited — the first 5 per 60 s window are logged in full with a `TOMO_UNHANDLED_REJECTION` marker, then one summary line per window carries the exact suppressed count — and a non-`Error` reason is rendered with `util.inspect` rather than as `[object Object]`. An **uncaught exception is logged and exits 1**: the state that produced it is undefined, continuing risks corrupting durable state, and a non-zero exit is the only thing launchd `KeepAlive` restarts (a deterministic crash therefore becomes a restart loop, bounded by the plist's `ThrottleInterval`). The marker `TOMO_UNCAUGHT_EXCEPTION` is written to stderr synchronously before pino is touched, since `process.exit` does not wait for the transport worker; the handler latches so a second exception on the way out cannot re-enter. One salvage step runs before the exit, `Agent.recordUnprocessedInboundOnCrash()` — a synchronous local transcript append that keeps #294's property (a received message never becomes a silent non-answer) through a crash; it reports, rather than caps, an overrun, because synchronous work cannot be preempted. Nothing else runs on that path: no `await`, no network, no channel handshake. Startup failure goes the same way: `cli.ts` runs commander's synchronous `parse()`, so a rejection of the async `start` action (say `agent.start()` failing because `imsg` lacks Full Disk Access after Telegram has begun polling) was an unhandled rejection — which the new handler would have *survived*, leaving a half-started daemon holding the pid file with no schedulers and no watch server. The start command now catches it and calls `raiseFatal(err, "startup")`, which takes the uncaught-exception path. The rate-limit summary is also flushed by an unref'd timer when a window ends quietly, so a burst that simply stops still reports its suppressed count; the bootstrap install no longer loads pino first (the module has no static `logger.ts` import); and a reason whose `util.inspect.custom` throws cannot break the handler on its way to the exit hook.

- **The daemon claims its PID file atomically as its first act, and `tomo stop` waits for the exit it reports.** The pid file was written at the *end* of startup, after config load, five `mkdir`s, a recursive skills copy, channel construction and `metricsExporter.start()`, with only an `existsSync` + `kill(pid, 0)` check at the top — so a login-time autostart racing a manual `tomo start` put both daemons past the check, and the file named only one of them. New `src/cli/pidfile.ts`: `acquirePidFile()` publishes a complete pid file by `link()` (EEXIST rather than overwrite, and never observable as existing-but-empty, which an `open(wx)` + `write` pair was — four of six racers "won" that way), refuses on a live holder, and takes over a stale one. Every pid-file mutation — the stale takeover, the release on exit, the reap a `tomo status` does — runs under a lock directory that is created only by atomic `rename` and reclaimed only by unlinking an abandoned instance's uniquely named owner entry, so two starters that both judge one stale file takeable cannot both take it over and a reader can never delete a claim a starting daemon has just re-taken; a live owner is never evicted by age. The pid file carries a second line — the process's start time and argv from `ps` — so a pid recycled after a SIGKILL is a *stale* file, not a permanent refusal (under `KeepAlive` that was a restart loop); every reader goes through `readPidFileRecord`, and nothing signals a pid whose identity no longer matches. `tomo stop` used to print "Stopped Tomo" on the strength of having sent SIGTERM (and, with the LaunchAgent plist on disk but the daemon started by hand, of having booted out a job that was not loaded); it now always falls through to the pid file, waits for the pid to go away on one shared 60 s budget (`DAEMON_STOP_TIMEOUT_MS`, also used by `tomo restart` and the LaunchAgent restart — 10 s reported a *healthy* mid-turn shutdown as a failure and advised `kill -9`), confirms identity once at the end, and exits 1 with the `kill -9` hint only when the daemon is really still there. `tomo restart` is bounded on the same budget instead of looping silently. Enabling autostart waits for the manual daemon to exit before bootstrapping launchd instead of unlinking its live claim mid-shutdown and starting a second daemon alongside it. Background `tomo start` waits until the pid file names its child (or the child exits, whose stderr is shown) before printing "started" — two concurrent starts both used to report success. `ThrottleInterval` (30 s) is added to the plist next to `KeepAlive`, and one `isPidAlive` replaces three copies that disagreed about EPERM (a cross-uid daemon was "dead" to two of them, so `tomo stop` declared success without sending anything and `tomo status` deleted its pid file).

- **The legacy `[[NL]]` marker can no longer reach a channel, and the model is no longer taught it.** `send_message` direct mode and `edit_message` sent model-authored text verbatim, skipping the `[[NL]]` rewrite every reply block gets — so on 2026-08-30 a cron morning brief sent through direct mode reached iMessage with `AI[[NL]]· …` in it. Both outlets now run the same `restoreLiteralNewlines` as the per-block path, and the transcript and pending note record the rewritten text, i.e. what was actually sent. The rewrite also absorbs spaces and tabs hugging the marker (`AI [[NL]] · item` becomes `AI\n· item`, not `AI \n · item`), and two adjacent markers are a blank line. The marker is deprecated: newlines have been formatting rather than message boundaries since #292/#293, so the system skill now says to write a real newline and describes `[[NL]]` as tolerated for back-compat only.
- **A cron job interrupted by a restart is no longer silently fired a second time.** `nextRunAt` only advanced when a run *completed* (`CronStore.markRun`), and the "don't re-dispatch this" guard was an in-memory `Set` on the scheduler — so a daemon that went down mid-run (a `tomo restart` issued by the job's own turn, a crash, a kill) came back up, found a job whose `nextRunAt` was still in the past, and ran it again from scratch. Observed 2026-08-20: a job fired at 01:30:00, restarted the daemon during its turn, and fired again at 01:30:39. It was idempotent, so nothing broke; a job that sends, orders, posts or submits would have done it twice, both times reporting success. Dispatch is now recorded durably *before* the agent turn starts: `markStarted` writes a fresh run token (`lastRunId`, alongside `lastStartedAt` for display), and completion acknowledges it (`lastCompletedRunId`). A token that was issued and never acknowledged is unambiguous evidence of a run that was dispatched and never reported back — a comparison of ids, not of clocks, so a machine whose time steps mid-turn cannot make a finished run look unfinished. `CronScheduler.start()` settles those before its first due-scan: a **recurring** job still fires, but with a `[resumed]` note in the `<tomo-event>` body naming the interrupted run's start time and telling the model to check state before repeating anything with side effects — one marked fire, not a silent duplicate, and the marker is persisted (`interruptedAt`) so it survives further restarts and is cleared by the dispatch that delivers it. A **one-shot** job is never re-fired at all: its single fire already happened and may well have succeeded, so it is disabled with the new `lastStatus: "interrupted"` for a human or the agent to inspect and re-enable deliberately. `lastStartedAt` and the interrupted status surface in `schedule_list`, `tomo cron`, `tomo config`, and the watch snapshot. Jobs written before this change carry no `lastRunId` and so read as never-interrupted — the pre-upgrade backlog is not retro-flagged. The normal path is untouched: an uninterrupted run marks started, runs, marks done, and keeps its drift-free cadence.

  Four things that shared the same shape were fixed with it. **Recovery is a precondition for scanning, not a best-effort step**: if it cannot persist, the due-scan is skipped and retried on the next poll, because a scan that runs before recovery lands is exactly the duplicate this change exists to prevent. **The turn's outcome and the recording of that outcome are now separate failures.** Previously a write error after a *successful* turn fell into the same `catch` as the turn itself and was recorded as `markRun("error")` — which, for a one-shot, is the failure-retry path: a completed order would be placed again five minutes later. The outcome is now captured first and persisted second, by a path that cannot invent an error; a failed write is retried on later ticks with the job held out of the scan, and after three attempts the job is parked as finished-but-unacked (disabled, outcome recorded, not deleted) rather than left looking due. **A cron run handed to an active summoned session is no longer reported complete when it is merely queued** — `handleCronMessage` takes a `waitForHandoff` option (set by the scheduler, not by the delegate path, which would deadlock on its own queue) so the job is advanced or deleted after the turn actually runs. And **a job interrupted on every restart is no longer resumed forever**: `resumeAttempts` is persisted, capped at three, and cleared by any run that reaches a successful outcome, after which the job is disabled with a logged reason instead of re-firing once per crash.

  One more correctness point. Every store write is now a three-way merge against the file as it stands at write time — baseline from our load, our edits, whatever is on disk now — so a `tomo cron` / `schedule_create` process holding a snapshot that predates a dispatch cannot erase `lastStartedAt` / `lastRunId` by saving its own unrelated edit. Field-level last-write-wins, deletions respected on both sides; it is not a lock, and two writers editing the *same* field still race.

  That merge is now backed by an optimistic `revision` counter in the file (distinct from the schema `version`, which stays at 1). A save merges against the revision it just read and re-checks that revision at the last possible moment — after the temp file is written, immediately before the rename that publishes it (`writeFileAtomicSync` gained a `beforeRename` hook for exactly this) — re-reading and re-merging if another process got there first, up to five attempts before reporting a write failure. The lost-update window shrinks from the whole read-modify-write to the rename syscall. `mergeWithDisk` also stopped conflating an absent field with an explicit `null`: clearing a field (`interruptedAt = null` once the `[resumed]` marker has been delivered, `nextRunAt = null` on disable) is an edit that must beat a concurrent non-null value, not be read as "we never touched this".

  **An unreadable jobs file is no longer read as an empty one.** `readStore` distinguishes ENOENT (a fresh install; empty, fine) from a read or parse failure, which now throws. The old catch-all returned `[]`, which meant a transient `EACCES` or a corrupt file let interrupted-run recovery report success with nothing to recover — and then fire the interrupted job unmarked once the file came back — and let the next save publish an empty store over the real jobs. A `CronStore` whose load failed still constructs (a read error must not take the daemon down at boot) but refuses to read or write until a later load succeeds, and the scheduler treats a failed recovery as recovery-not-done and retries on the next poll. Two gaps closed in review: each entry of the `jobs` array is checked for the shape of a job record (`{"jobs":[null]}` used to pass the array check and surface as a `TypeError` from the scheduler's due-scan rather than as a read failure), and a reload that fails *after* a successful one now records the failure too — previously only the constructor did, so a file that went bad while the daemon was running left `list()` serving the earlier snapshot and the degrading surfaces reporting a healthy store.

  The surfaces that read the store were updated to match, each in the way that suits it. `tomo cron` prints `cron store unreadable: <path>` and exits non-zero instead of dumping a stack trace. `tomo status` **degrades instead of dying**: the report still renders — daemon state, config issues and channels are exactly what someone runs it for when something is broken — with the cron section reading `Scheduled tasks: store unreadable (<path>)` rather than the `none` that started this whole bug. The watch snapshot and the Prometheus scrape degrade the same way: a corrupt jobs.json used to drop the `tomo watch` client (the snapshot threw) and 500 the whole `/metrics` scrape, hiding everything else those surfaces report; they now serve an empty cron section and log a warning, while the scheduler keeps handling the same failure properly by refusing to scan until it can read the store. The interactive `tomo config` cron view and the identity migration report the same line and return to the menu rather than killing the process mid-configuration, with the migration explicitly saying that nothing was re-keyed. `schedule_create` no longer reports a store failure as `invalid schedule "..."`: the schedule is now validated on its own (parse plus a trial `computeNextRun`, which is where croner rejects a malformed expression) and the store write is reported separately, so the agent does not respond to an unreadable file by rewriting a perfectly good schedule string. `schedule_create` also validates the delivery target now: the `session` argument must be a `dm:<identity>` key or a `<channel>:<chatId>` key, with no control characters or stray whitespace. It was free text written straight into the store, so a malformed target bought a job that could never deliver — discovered days later when the reminder did not arrive.

  One more correctness fix in the same area: `setEnabled(true)` used to acknowledge the run token unconditionally while its sibling clears were gated on `lastStatus === "interrupted"`. A disable/enable round-trip *while a run was in flight* — an easy thing to do from `tomo config` — erased the evidence that the job had been dispatched, and the next restart re-fired it with no `[resumed]` marker (and, for a one-shot, re-fired it at all). Every clear now lives inside the settled-interrupted branch.

- **`tomo config` no longer writes `{}` over a config file it could not parse.** `loadConfig()` returned `{}` for *any* failure — a missing file, `EACCES`, or the trailing comma a hand-edit leaves behind — and every `configXxx()` is `loadConfig()` → mutate one key → `saveConfig(cfg)`. So editing the model on an unparseable config persisted `{"model":"claude-sonnet-5"}`: the Telegram token, allowlist, identities, `groupSecret`, `mcpServers`, `plugins`, `auth.apiKey` and `litellm` were all gone, and the CLI printed a success message. Recovery was exactly one save deep, because `backupFileIfExistsSync` copies *the file being saved* into the single `.bak` — so the second save replaced the good backup with the clobbered one. `loadConfig` now distinguishes absent (a fresh install; `{}`, fine) from unreadable, which throws a typed `ConfigReadError` carrying the underlying syntax error. `saveConfig` re-checks the file on disk at write time rather than trusting the load — the config UI is a long-lived interactive session and the file can be edited underneath it — and that check runs *before* the backup rotation, so a `.bak` is only ever taken from a file that parsed. `tomo config` reports the parse error, names the `.bak` to restore from (or says there is none), and exits non-zero. It does not quit outright: the submenus that never read `config.json` — scheduled tasks, cost analysis, autostart — stay on the menu, and only the read-modify-write ones are withdrawn. Two of those also had a side effect ordered *before* their save — clearing an SDK session, and moving an identity's cron jobs back to the fallback key — so a refused save used to leave the side effect committed; both now save first, and both wrap the side effect so a store that refuses reports the truth — the config change landed — instead of a stack trace. `tomo init --force` had the same content-blind rotation: it overwrites the config on purpose, so the `.bak` is the only surviving copy, and a corrupt config used to replace a good backup there too; it now only rotates a file that parses. The startup-time warning at `config/index.ts` was already telling the user the daemon would refuse to start; it just never mentioned that saving would erase the file, and nothing blocked the save.



- **A compact, prune or transcript rotation no longer deletes JSONL lines it could not parse.** `parseJsonl` skips malformed lines, which is the right tolerance for a read-only consumer inspecting a file another process is writing — but three call sites are read-modify-**rewrite** (`lcm/compact.ts`, `lcm/prune-tools.ts`, and `SessionStore.maybeRotateTranscript`) and emitted only what parsed, so anything dropped was not in the output and the rename made that permanent. The tail case was already handled (`hasPartialTail` aborts the rewrite), but a *mid-file* tear — a power loss, with a later append landing behind it — survives that check, and the next `tomo lcm daily` erased it. If the torn line was an assistant `tool_use`, the following `tool_result`'s `tool_use_id` was left dangling. `parseJsonl` now takes `{ preserveUnparseable: true }`, which emits an opaque carrier for each unparseable line in its original position, and `serializeJsonlRecord` writes it back byte-for-byte; the carrier's key is a `Symbol`, so a line that ever leaked into a plain `JSON.stringify` would be caught rather than silently becoming `{}`. Every rewriting caller opts in — including the late-append splice and the post-rename old-inode drain, which write into the same files — while every read-only caller (`lcm/blocks.ts`, `lcm/stats.ts`, the transcript readers) keeps the old skip-and-continue behaviour unchanged. Carried lines have no `type`, `uuid`, `message` or `timestamp`, so they never enter the conversation index, never anchor or expand a compaction range, and are never archived as if they had been summarized: a torn line inside a collapsed range is re-emitted immediately after the summary that replaced it, and rotation keeps it in the active transcript rather than filing it under a month it never claimed. Preserve-verbatim was chosen over refuse-to-rewrite at all three sites: an LCM chain that stops compacting because of one bad byte is its own outage, and the content is opaque either way. The opt-in is enforced by the type system rather than by convention — `parseJsonl` is overloaded so that asking to preserve widens the element type to `(T | RawJsonlLine)`, which means a caller that opts in cannot read a field without narrowing first; adding the overload alone produced fifteen compile errors at exactly the places that had been relying on carriers happening to have no `type`. Each preserved line is logged once with its session, index and byte length, because a line nobody can read is worth surfacing even when it is being kept, and `tomo lcm compact --drop-unparseable` / `tomo lcm prune-tools --drop-unparseable` are the documented, off-by-default escape hatch for corruption that is jamming a session. One more hardening: a line that is *valid* JSON but not an object — `null` above all, which parses fine and then throws on every `evt.type` read — is now carried verbatim too rather than handed out as a record. Read-only callers are unchanged and still receive it. The round-trip guarantee is per-line, not whole-file: blank lines are skipped in every mode, since JSONL gives them no meaning and the SDK never writes them. A preserved line is by construction still there on the next rewrite, so `reportRawJsonlLines` now warns once per (file, line) and again only after 24 h, with repeats at debug; a dry run — the per-turn nudge check calls `pruneTools({ dryRun: true })` — logs at debug and does not count as reported. `--drop-unparseable` is exposed on the `daily`/`weekly`/`monthly`/`yearly` rollup subcommands too, not only `compact` and `prune-tools`. A bare array line is carried like `null`. The plain `T[]` overload is offered only when the flag is absent or a literal `false`, so a non-literal boolean resolves to the widened union instead of quietly to `T[]`. `computeContextStats`, `resolveTimeRange` and the block scan skip `null`/array lines instead of throwing on `e.type`.


- **A session registry that could not be read is no longer persisted as an empty one.** `loadRegistry()` had a bare `catch` that set `this.registry = []`, so *any* failure — a JSON parse error, a transient `EMFILE`/`EIO`, a half-restored file — was indistinguishable from "there are no sessions". Every mutator (`setSdkSessionId`, `updateStats`, `touchSession`, `setChatTitle`, `addParticipant`, `clearSdkSessionId`) is `loadRegistry()` followed by `saveRegistry()`, and `saveRegistry` writes `this.registry` atomically over the real file: one unreadable instant, and the next turn's `updateStats()` published `{version:1,sessions:[]}`. Every conversation then cold-started a new SDK session, the old JSONLs were orphaned, and `cleanupExpired` could never TTL them because their entries were gone — months of continuity, irrecoverable, with no log line at all. `loadRegistry` now distinguishes ENOENT (a fresh install or `tomo sessions clear`; legitimately empty) from a read or parse failure, including `statSync`'s own errors, and validates that `sessions` is an array. On failure it changes nothing in memory: the last state we successfully loaded keeps serving reads, because telling a caller there is no SDK session is what starts a new one. Mutators are then classed by what refusing costs. **Bookkeeping** — `updateStats`, `touchSession`, `setChatTitle`, `addParticipant`, `setReplyTarget` — never throws: it returns without touching memory or disk and logs once per streak. These sit on the inbound path (`addParticipant` runs before the message is appended to the transcript, and the rejection is swallowed upstream) and on the turn-completion path (`updateStats` runs after the model has already answered), so a throw there silently drops an inbound message or fails a turn that succeeded — and a permanently corrupt registry would mean *every* turn fails. **Link-changing** — `setSdkSessionId`, `clearSdkSessionId`, `retireSdkSessionId`, `migrateSessionKey` — refuses hard with a typed `SessionRegistryReadError`, because a link invented from a registry we could not read orphans a JSONL for good. `cleanupExpired`, which unlinks SDK JSONLs and runs from the constructor, skips entirely. Every guard runs after the load and *before* any mutation, so a refused call leaves nothing un-persisted behind: `getSdkSessionId` is unchanged after a refused `clearSdkSessionId`, and a skipped `setChatTitle` on an unseen key does not leak the metadata stub `ensureActiveEntry` would have created. Reads log once per streak that they are answering from the last known-good state — a short-lived CLI store that never had a good read reports an empty list, which is now visible rather than silent. A 0-byte registry is no longer a permanent refusal (`JSON.parse("")` throws): it reads as empty when no sessions are held, so an interrupted first write on a fresh install self-heals, and is refused when sessions are held, because something truncated a real registry. Recovery from the latter is to delete the file, which reads as ENOENT. The stat cache — which normally lets a mutator skip re-reading a file it just wrote — is bypassed while the load is failing, so the first readable moment is picked up immediately and normal operation resumes on its own. Constructing a `SessionStore` still succeeds when the registry is unreadable: a read error at boot must not take the daemon down, it just cannot write anything until the file comes back. Two places where the link-changing refusal escaped onto a path that must not fail are now best-effort: `router.resolve()` runs the one-time `dm:` migration on every inbound message, and its throw lost the message (the Telegram rejection is swallowed, the iMessage cursor never advances) — it now logs and routes the message to the existing channel-scoped session, deliberately not to the unified key (a turn there would cold-start a `dm:` session and block the migration for good), and retries on the next message; and `recordTurnCompletion` captures the SDK session id after the model has already answered — a refusal there now logs, keeps the answer, and remembers the key so the link is re-attempted after the next turn even once the registry's last-good state reports some other link. Bookkeeping saves (`updateStats`, `touchSession`, `setChatTitle`, `addParticipant`, `setReplyTarget`) are now best-effort for *any* save error, not only the read refusal: an ENOSPC or EACCES on the sessions directory is logged once per streak and the change stays in memory for the next successful save, rather than failing a turn that succeeded.
...
- **A `thinking` block with text in it is delivered as a message when `showThinking` is off.** With the flag off the SDK runs thinking `display: "omitted"`, which strips the reasoning and leaves a signature-only block whose text is empty — so an empty thinking block is the normal case and is still dropped, silently. A NON-EMPTY one under `omitted` is not reasoning that leaked; it is the model having written a reply in the wrong block type, and it is now rendered exactly like a `text` block: no `💭` prefix, the same scaffold-leak filter, the same bare-`NO_REPLY` drop, the same `MEDIA:`/`STICKER:` extraction and `[[NL]]` rewrite, shipped as its own message the instant the block completes. The evidence: one session's SDK transcript for 2026-08-28, flag off, held 173 thinking blocks with a 0-char `thinking` string and 21 with text — and all 21 were prose aimed at the owner (a reply after a tool result, a progress line after a steered message), six of them answers he was waiting for and never got. None was leaked reasoning. The rule is decided from block type and length alone, never from what the prose says, so it does not reintroduce the content inspection #292 removed. `showThinking: true` is unchanged (thinking delivered, `💭`-prefixed). Turns with `suppressDelivery` (heartbeats, LCM nudges) still ship nothing — this changes how a block is rendered, not whether a turn may speak. Each such delivery logs once at warn with the delivered length, so the rate at which the model misplaces a reply stays visible. Unowned (SDK-initiated) turns take the same path: the shape that lost a ~700-char DM reply at 08:33 that day now reaches the session's default target.
...
- **Credentials no longer reach `tomo status`, the launchd error log, or `tomo.log`.** Three separate paths, one shared rule in the new `src/redact.ts`.

  **`configIssues` printed the value it was complaining about.** `describeValue` was a bare `JSON.stringify`, and `parseChannels` validates a channel entry as one object — so a single hand-edit mistake in a sibling field (`"allowlist": "123456789"` instead of an array, the easy one) stringified the whole entry, `token` included. That string is rendered in three places: `tomo status`, the `tomo config` banner, and `assertConfigValid()`'s throw, which `start.ts` `console.error`s — under launchd that appends the token to `~/.tomo/logs/launchd.err.log`, a file that goes into bug reports. `litellm.apiKey` had the same path, and `groupSecret` leaked whenever it was the value that failed validation. Field names are matched by whole segment rather than substring, so a token COUNT (`tokens`, `inputTokens`, `tokenCount`) is never mistaken for a token. Secret-named fields are reduced to `***` plus their last four characters, and only for values long enough (12 characters) that four are a fingerprint rather than a substantial share of the secret — below that the value is masked completely. The message stays actionable: the label names the entry, the zod path names the failing field, and the operator can still tell *which* token is on disk. Non-secret siblings print verbatim, because seeing what you actually typed is the point.

  **The pino message carried live tool output.** This is the path that leaked in ordinary use, not just on a misconfiguration: `agent/live-session.ts` logs `summarizeToolResult(...)` — the first 500 characters of *every tool result* — as the log message at `info`, and `summarizeToolInput(...)` does the same for tool arguments. A single `Read ~/.tomo/config.json`, or any command that prints the MCP OAuth token store, wrote live credentials into `~/.tomo/logs/tomo.log`, defeating the `0600` that the token store carefully sets. Scrubbing happens inside `summarizeToolInput`/`summarizeToolResult` themselves, which covers the watch bus as well as the log — `publishToolStart` sends the same summary to the TUI without going through pino. A message is a free-text string with no key to match on, so `scrubSecretValues` matches on the *shape of the value* — Telegram bot tokens, `sk-`/`sk-ant-`, `ghp_`/`github_pat_`, Slack, AWS key ids, Google keys, JWTs, `Authorization`/`Cookie`/`Set-Cookie` headers and bare `Bearer`/`Basic` credentials, and the `access_token:`/`client_secret=` key-value forms — keeping the key so the line still reads and replacing only the value. It also runs from pino's `logMethod` hook, so it covers every other log call in the daemon, present and future. It is deliberately constrained so it cannot mangle prose: `Bearer`/`Basic` are matched case-sensitively and only in front of something that looks like a credential (8+ characters with a digit, or 16+ of anything, never ending in punctuation), header rules consume the scheme plus one token rather than the rest of the line, and `token: null`, `secret: the meeting is at 3` and `Authorization: required for this endpoint` all survive intact — real lines from `tomo.log` like "assuming basic features are supported" and "access token expired." are pinned as tests, because a scrubber that damages logs is a scrubber that gets turned off.

  **`src/logger.ts` had no `redact` config at all.** It now has three layers. A `formatters.log` hook applies the name rule at *any* depth, because a ladder of literal `redact.paths` can always be out-nested — `{config:{channels:{telegram:{token}}}}` is four levels, an MCP entry nests `mcpServers.<name>.headers.Authorization`. A wrapped `err` serializer handles error trees, which need their own pass for three reasons: `formatters.log` runs *before* serializers so it only ever sees a raw `Error`; `pino.stdSerializers.err` output has a non-`Object` prototype that a plain-object walker rejects, which left `err.response.data.config.headers.Authorization` exposed at depth 6; and the credential is often in `message`/`stack` under no key at all, including inside an `AggregateError`'s sub-errors. It wraps rather than replaces `stdSerializers.err`, so stacks survive. `redact.paths` is kept only as a short belt-and-braces list of header casings: at five nested wildcards it was 115 paths and measured **42 µs per log line against 1.1 µs for the deep hook alone** — a 39x tax, on a logger that runs at `debug` by default, to find things the hook had already found. The deep hook returns the record untouched, by identity, when nothing changed, so a record with nothing to redact is not cloned and a class instance is not flattened for no reason.

  The deep hook runs in `formatters.log`, which is **outside** pino's guarded serialization, so it is written to be unable to take a log call down: reading properties one at a time rather than through `Object.entries` (which invokes getters, and one throwing getter used to kill the log call), classifying values inside a `try` (`instanceof` performs `getPrototypeOf`, which throws on a revoked `Proxy`), capping depth at 32 (a 60k-deep object overflowed the stack), and falling back to the record plus a one-time warning if anything unforeseen still escapes. It also scrubs string *leaves*, not just field names — `{ dbUrl: "postgres://admin:S3cret@host" }` and `{ header: "Authorization: Bearer sk-ant-…" }` carry credentials under names no rule can match, and `imessage-imsg.ts` logs child-process RPC params — with a value-shaped rule for URL userinfo that keeps the scheme and username. Strings under 8 characters and beyond 8 KB are skipped, and the bot-token pattern is bounded to 20 digits after an unbounded `\d{6,}` was measured backtracking for 910 ms on a 32k-digit input.

  What counts as a secret lives in one place: name contains `token`/`secret`/`password`/`apikey`/`authorization`/`cookie`/`credential` (case- and separator-insensitive, so `Token`, `API_KEY` and `X-Api-Key` all match), or is a `…Key` with a credential qualifier. Deliberately **not** matched: a bare `key`, and `sessionKey`/`storeKey`/`chatKey`. In this codebase those are routing identifiers — `key` is the session key in `agent.ts`'s elicitation logging — and blanking them would cost the diagnostics an operator reads the log for while protecting nothing, since no config field is named `key`.
...
- **A picture that fails to send no longer takes its caption with it — and the transcript records exactly what shipped.** The delivery pipeline checks a `MEDIA:` path with `existsSync` and the channel opens it again later; a file that disappears in that window fails the send outright, and `captionSent` was set on having *called* `send`, not on the send resolving. A block of `Here's the chart` + `MEDIA:/tmp/chart.png` therefore lost the image **and** the text, plus any `STICKER:` after it in the same block — on Telegram, where grammY throws, and on iMessage, where `sendAttachment` logged "not found" and returned normally, so the block was recorded as delivered. Both channels now report a missing file as a typed, *definite* pre-flight failure (`AttachmentUnreadableError`, thrown before any request goes out), and the pipeline owns the fallback: the caption goes out as a normal text send (inheriting the reply target the picture could not take), stickers still ship, and the block rejects with a `PartialDeliveryError` carrying the per-piece outcome, which the transcript records as the caption verbatim followed by `[delivery failed] MEDIA:…` — never the whole block marked failed when the owner has the text on his phone. The fallback is deliberately limited to failures that provably happened before dispatch (the typed error, or a local file-system code down the wrapping chain): an *ambiguous* failure — a timeout, a dead `imsg` child, a dropped connection — may already have landed the captioned picture, so re-sending the caption would be the same duplicate both channels already refuse to risk on their own retries. In that case the caption is left alone, recorded as not known to have shipped, and the reply target is retired rather than reoffered.

- **One corrupt record no longer truncates a transcript recall.** `searchTranscript` scans newest→oldest and `break outer`s at the first record below the requested lower bound — which abandons the rest of the active transcript *and* every monthly rotation archive behind it. Both bounds coerced an unusable value into a real position: a missing `seq` became `0`, and a `timestamp: 0` (or a seconds-precision epoch written where milliseconds were expected) was read as genuinely ancient. Either one, anywhere in a file, ended the whole search — and `recall_conversation`'s `after` argument is exactly that lower bound, so the model was told "that's all I could find" about a conversation still fully on disk, with no error anywhere. A record whose position cannot be established is now skipped, never used to end the scan, and the upper bounds (`toSeq`/`toTime`) exclude it instead of silently accepting it. The early exit for records that are genuinely outside the window is unchanged.
