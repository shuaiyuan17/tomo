...
- **Two people no longer share one transcript file.** `SessionStore` named a transcript by replacing every character outside `[A-Za-z0-9_-]` with `_`, which is many-to-one: `imessage:any;-;alex.smith@example.com` and `imessage:any;-;alex_smith@example.com` are two different contacts that both resolved to `imessage_any_-_alex_smith_example_com.jsonl`, and with them the whole `_archive_<name>_<YYYY-MM>.jsonl` set. Everything downstream followed the filename rather than the key — `get()` loaded one person's tail into the other's session, `append()` interleaved them, `searchTranscript` and `recall_conversation` returned one contact's messages when asked about the other, and rotation rewrote the shared file for whichever key happened to trigger it. Any two keys differing only in a character the replacement flattens (`.` vs `_`, `@` vs `+`, a Unicode name against its punctuation twin) hit it.
  ...
  Filenames are now derived by `transcriptFileStem()`, which is injective. A key drawn only from `[A-Za-z0-9:-]` keeps the name it already has — `dm:alice`, `telegram:123`, `telegram:-100123` and every other identity or Telegram key never move — and anything else becomes `<legacy stem>.<12 hex chars of sha256(key)>`, hashed over the *full key* so the two addresses above differ. `_` is deliberately not in that stable set: a `_` in the key is indistinguishable from an encoded `:`, so `dm:a_b` takes a suffix too. `.` is outside the legacy safe set, so a suffixed stem can never equal an unsuffixed one, and the archive prefix scheme keeps working untouched.

  Existing files are carried over **lazily, on the first touch of a key**, and only when ownership is unambiguous. If no other key we know of maps to the same legacy stem — checked against every registry entry, active and unlinked, by `channelKey` *and* `migratedFrom`, plus the in-memory session cache — the active file and each monthly archive are renamed under an advisory lock, and history, `createdAt` and search keep covering them. If another key does, nothing is renamed and nothing is guessed: that file is the only record of the interleaved history, so it stays exactly where it is for manual inspection (its records carry `channel` and `senderName`), one warning names the colliding keys and the file, and each session continues in its own file from then on. The migration never throws on the message path — a lock held by another process, an EACCES, a rename that loses a race all degrade to "don't migrate", logged once, retried on the next daemon start — and `migrateSessionKey` settles the old key's filenames before it re-keys the entry, so identity unification still carries the transcript and its archives across.
