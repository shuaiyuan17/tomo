...
- **A picture that fails to send no longer takes its caption with it.** The delivery pipeline checks a `MEDIA:` path with `existsSync` and the channel opens it again later; a file that disappears in that window fails the send outright — grammY's `InputFile` raises `ENOENT` at request time, and `ImsgChannel` re-checks the path — and `captionSent` was set on having *called* `send`, not on the send resolving. A block of `Here's the chart` + `MEDIA:/tmp/chart.png` therefore lost the image **and** the text, plus any `STICKER:` after it in the same block, on both channels. The flag now tracks what actually shipped: the caption falls through to a normal text send (inheriting the reply target the picture could not take), stickers still ship, and the original failure is re-thrown afterwards so the block is still recorded with the delivery-failed marker — everything deliverable is delivered, and nothing claims a delivery that did not happen. `ImsgChannel.sendAttachment` additionally delivers the caption itself when it finds the file missing, rather than returning with the caption unsent.
