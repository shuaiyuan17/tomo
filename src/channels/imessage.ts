...
export class BlueBubblesChannel implements Channel {
  ...
  private async handleWebhookEvent(payload: Record<string, unknown>): Promise<void> {
    ...
    if (text.startsWith("/")) {
      ...
      if (command === "new" || command === "model" || command === "restore" || command === "login" || command === "mcp" || command === "status" || command === "cost" || command === "usage" || command === "pet" || command === "summon" || command === "dismiss" || command === "pause" || command === "resume") {
