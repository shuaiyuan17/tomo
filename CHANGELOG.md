...
- **Credentials no longer reach `tomo status`, the launchd error log, or `tomo.log`.** Three separate paths, one shared rule in the new `src/redact.ts`.
  ...
  **The pino message carried live tool output.** This is the path that leaked in ordinary use, not just on a misconfiguration: `agent/live-session.ts` logs `summarizeToolResult(...)` — the first 500 characters of *every tool result* — as the log message at `info`, and `summarizeToolInput(...)` does the same for tool arguments. A single `Read ~/.tomo/config.json`, or any command that prints the MCP OAuth token store, wrote live credentials into `~/.tomo/logs/tomo.log`, defeating the `0600` that the token store carefully sets. A message is a free-text string with no key to match on, so `scrubSecretValues` matches on the *shape of the value* — Telegram bot tokens, `sk-`/`sk-ant-`, `ghp_`/`github_pat_`, Slack, AWS key ids, Google keys, JWTs, `Bearer`/`Basic`/`Token` headers, and the `access_token:`/`client_secret=` key-value forms — keeping the key so the line still reads and replacing only the value. It runs from pino's `logMethod` hook, so it covers every log call in the daemon, present and future, and the watch TUI's issue feed with it.

  **`src/logger.ts` had no `redact` config at all.** It now has three layers: `redact.paths` for the enumerated names (a fast, well-tested path), a `formatters.log` hook that applies the same name rule at *any* depth (a ladder of literal paths can always be out-nested — `{config:{channels:{telegram:{token}}}}` is four levels, an MCP entry nests `mcpServers.<name>.headers.Authorization`), and a wrapped `err` serializer, because an error is the other way a credential arrives: grammY echoes the failing request URL with the bot token glued to `bot`, and an axios-shaped error carries `config.headers.Authorization`. The serializer wraps rather than replaces `pino.stdSerializers.err`, so stacks survive. The deep hook returns the record untouched when there is nothing to redact, which is almost every line.
