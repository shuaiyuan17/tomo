...
- **A key is marked migrated only once the migration finished.** An attempt that could not complete (transcript lock held by another process, EACCES, an unreadable ownership source) is retried, throttled by `TRANSCRIPT_MIGRATION_RETRY_MS`, and reported by `store.migrationStatus()` (`{settled, deferred, ambiguous, sidecars}`, logged at info on daemon start). Marking first and then refusing the both-exist case is what left a whole history on disk but unreachable from `loadTranscript` / `searchTranscript` / `transcriptCreatedAt` / `getLastSeq`.
