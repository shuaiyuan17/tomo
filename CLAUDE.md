...
Files written under the old scheme are migrated **lazily, on the first touch of a key** (`ensureTranscriptMigrated`, called from `get()`, under the `_transcripts.lock` advisory lock, and never throws on the message path). Ownership decides: if no other key we know of maps to the same legacy stem, the active file and every archive are carried over. If one does, the legacy file is AMBIGUOUS: it is left exactly where it is as the only record of the mixed history, one warn names the colliding keys, and each key starts fresh under its own new name.

Two things that look like details and are not:

- **Ownership is read from `_legacy_stems.json`, not inferred.** The registry forgets keys — `cleanupExpired` drops an unlinked entry after 30 days while the transcript file stays, `clearSdkSessionId` deletes a metadata-only stub, `migrateSessionKey` re-keys in place — so all three write the key's legacy stem to that ledger *before* forgetting it, and `otherKeysSharingLegacyStem` reads the ledger as well as the registry and the session cache. Any new path that removes a registry entry must do the same, or a shared legacy file becomes silently adoptable again.
- **A key is marked migrated only once the migration finished.** An attempt that could not complete (transcript lock held by another process, EACCES) is retried, throttled by `TRANSCRIPT_MIGRATION_RETRY_MS`; and if the new-stem file already exists the legacy file is FOLDED into its head (seq numbers of the newer records shifted up) rather than abandoned. Marking first and refusing the both-exist case is what left a whole history on disk but unreachable from `loadTranscript` / `searchTranscript` / `transcriptCreatedAt` / `getLastSeq`.
