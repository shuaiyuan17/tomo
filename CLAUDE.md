...
Any code path that rewrites a JSONL file it just read must call `parseJsonl(text, { preserveUnparseable: true })` and write records back with `serializeJsonlRecord` — the default tolerance drops malformed lines, which is fine for a reader and permanent data loss for a rewriter. Read-only consumers keep the default.
