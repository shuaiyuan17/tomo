...
- Delivery: a non-empty `thinking` block is no longer shipped as a message when `showThinking` is off. Under thinking `display: "omitted"` the text that arrives in such a block is now a summary of what the model wrote, not the words themselves, and shipping it delivered paraphrases — one asserting a next step the model never planned — in place of the reply the model had composed. The block is dropped, kept in the transcript, and logged with its length (`non-empty thinking block dropped`); the 2026-08-28 rule that shipped it (#297) is reversed. With `showThinking` on, thinking still ships with the 💭 marker.
- Agent profiles: a write verb is now judged by the words of its own command segment (split at `;`, `|`, `&&`, `||`), not the whole line — `xcodebuild … | sed 's/^/  /'; rm -rf /tmp/scratch` was refused because sed's lone `/` is an ancestor of every denyPath. A git segment's relative pathspecs resolve against its `-C <dir>` / `--work-tree=<dir>` directory, so a worktree agent whose process cwd is the shared checkout can `git -C /tmp/wt commit -- Sources/A.swift`; `-C` pointed outside the writeRoots is still a write outside them, and `-C` counts as naming the place for readonly mode.
...
- **Per-agent permission scoping.** A new optional `agentProfiles` map in `~/.tomo/config.json` fences a subagent type into `writeRoots`, two deny lists and one of four Bash modes (`none` / `readonly` / `worktree` / `full`), enforced by a `PreToolUse` hook keyed off the SDK's `agent_type`. Every session runs `bypassPermissions` and the SDK propagates that into subagents, so until now an agent declared "read-only reviewer" had a full shell on the whole machine — `AgentDefinition` scopes tools and has no notion of a path.
  ...
  The two deny lists are not the same rule. `denyPaths` fences **writes** — a reviewer's job is to read the checkouts it must never modify, so `git -C ~/Developer/bloom log` is allowed and `git -C ~/Developer/bloom commit` is not — and a destructive verb aimed at an *ancestor* of one is denied too. `denyReadPaths` is the secrets list: any mention by any tool, including `Read`/`Grep`/`Glob`. `bash: "full"` switches `writeRoots` off for the file-writing tools as well as for Bash, leaving the deny lists as the whole policy.

  Relative paths are the interesting case, because the daemon is never told where a subagent's worktree is. They are **never judged on the allow side** (so `mkdir -p Sources/New`, `rm -rf build/Old` and `git commit -m msg` work in `worktree` mode), and **are** resolved against the workspace on the deny side (so `rm -rf memory`, `rm -rf *` and `git clean -fdx` are refused). A `worktree`-mode agent is fenced from the workspace, not from its own worktree; that limit is documented rather than papered over.

  `readonly` does not mean "no writes" — it means a write has to NAME its destination and that destination has to be inside `writeRoots`. `touch /tmp/marker` is allowed when `/tmp` is a writeRoot; `rm -rf x` and `rm -rf *` are not, because nothing in the daemon can say where they land. That is the single clause separating it from `worktree`, which trusts the agent with its own unnamed cwd.

  The main thread is untouched, and an agent type with no profile is untouched — fail-open, with one `warn` per session per type and every subagent Bash call logged at `info`, so the real surface can be measured before that default is flipped. A bad `writeRoots` entry is a warning (it only narrows, and it keeps an unset `$TMPDIR` from bricking startup); a bad deny entry is a fatal config issue (it would widen). It is policy, not a sandbox: `readonly` allows `$(…)`, backticks and pipes on purpose, because refusing substitution refuses the reviewer. See the README section for the shape and the caveats.
...
- **Two people no longer share one transcript file.** `SessionStore` named a transcript by replacing every character outside `[A-Za-z0-9_-]` with `_`, which is many-to-one: `imessage:any;-;alex.smith@example.com` and `imessage:any;-;alex_smith@example.com` are two different contacts that both resolved to `imessage_any_-_alex_smith_example_com.jsonl`, and with them the whole `_archive_<name>_<YYYY-MM>.jsonl` set. Everything downstream followed the filename rather than the key — `get()` loaded one person's tail into the other's session, `append()` interleaved them, `searchTranscript` and `recall_conversation` returned one contact's messages when asked about the other, and rotation rewrote the shared file for whichever key happened to trigger it. Any two keys differing only in a character the replacement flattens (`.` vs `_`, `@` vs `+`, a Unicode name against its punctuation twin) hit it.

  Filenames are now derived by `transcriptFileStem()`, which is injective — and injective **up to case folding**, because the filesystem underneath usually is not case-sensitive (APFS and NTFS both default to insensitive). A key drawn only from `[a-z0-9:-]` keeps the name it already has — `dm:shuai`, `telegram:123`, `telegram:-100123`, `heartbeat` and every other all-lowercase key never move — and anything else becomes `<legacy stem>.<12 hex chars of sha256(key)>`, hashed over the *full key* so the two addresses above differ. Two characters are deliberately outside that stable set. `_`, because a `_` in the key is indistinguishable from an encoded `:`, so `dm:a_b` takes a suffix too. And **uppercase**, because `imessage:AbC` and `imessage:abc` would otherwise be handed `imessage_AbC` and `imessage_abc` — the same file on a Mac, i.e. the original leak with extra steps. `.` is outside the legacy safe set, so a suffixed stem can never equal an unsuffixed one (the hash's hex digits are lowercase, so that holds after folding too).

  Note the promise is about the *alphabet*, not the prefix: `dm:*` and `telegram:*` keys keep their filenames because the ones Tomo constructs are lowercase (`dmSessionKeyForIdentity` lowercases; chat ids are numeric), but an identity configured as `Shuai Yuan` yields `dm:shuai yuan`, whose file does move — and is migrated like any other.

  Existing files are carried over **lazily, on the first touch of a key**, and only when ownership is unambiguous. Ownership is not inferred from the live registry, because the registry forgets: `cleanupExpired` drops an unlinked entry 30 days on while the transcript file stays, `clearSdkSessionId` removes a metadata-only stub outright, and `migrateSessionKey` re-keys an entry in place. All three now write the key's legacy filename stem into a small `_legacy_stems.json` ledger **before** forgetting it, and the ownership check reads that ledger alongside the registry (active and unlinked entries, `channelKey` *and* `migratedFrom`) and the in-memory session cache. Without it, a legacy file shared by two iMessage addresses was silently adopted by whichever key outlived the other, and `get()` then served one person's history to the other with no warning at all. A key whose partner aged out *before* this ledger existed is genuinely unknowable and is documented as such rather than papered over.

  When ownership is ambiguous, nothing is moved and nothing is guessed: that file is the only record of the interleaved history, so it stays exactly where it is for manual inspection (its records carry `channel` and `senderName`), one warning names the colliding keys and the file, and each session continues in its own file from then on.

  **Ownership is compared case-folded, and it is checked for every key.** `imessage:A.b` and `imessage:a.b` have legacy stems differing only in case, which is *one file* on APFS and NTFS — comparing the stems verbatim answered "sole owner" for a file two keys share. And the keys that *keep* their legacy stem are checked too: `dm:a:b` keeps `dm_a_b.jsonl` while `dm:a_b` takes a hash suffix, so the stable key is the one left sitting on the shared file, and skipping the check there left it reading (and appending to) a mixed transcript forever. Such a key cannot simply "leave the file where it is", because that filename is its own — so the shared family is renamed to `dm_a_b.ambiguous-<ts>.jsonl`, which nothing reads and a human can: unreachable but preserved beats silently mixed. That parked file is also the *record* that the decision was taken, which it has to be — the fresh file the key opens next has the very same name, so without the marker every restart would park the new history too.

  **Nothing is adopted on a source that could not be read.** An existing `_sessions.json` or `_legacy_stems.json` that will not parse now defers every key's migration instead of reading as "no other owner recorded"; a corrupt ledger is quarantined to `_legacy_stems.json.corrupt-<ts>` before anything is rewritten, never overwritten in place. A collision detected from *any* source — including this process's in-memory session cache, which the next process does not have — is written to the ledger the moment it is seen. And a path that is about to *forget* a key now refuses to when the ledger write fails: `cleanupExpired` keeps the entries, `clearSdkSessionId` keeps the metadata-only stub (without TTL-ing it, which would be the same forgetting 30 days later), and `migrateSessionKey` refuses the re-key. A session that cannot be unified is loud and retried; a missing ledger row is silent and permanent.

  The migration never throws on the message path — a lock held by another process, an EACCES, a rename that loses a race all degrade to "don't migrate", logged once. Crucially it is also never *recorded* as done in that case: marking the key first meant one transcript-lock timeout was permanent. The message that triggered it went on to `append()` under the new stem, both files then existed, and the both-exist branch left the legacy file alone forever — so every message older than the failed migration was gone from history, search, `createdAt` and seq allocation while sitting intact on disk. Now an incomplete attempt is retried (throttled, so a held lock does not cost a second per message), and **the migration only ever renames**: when the destination name is already taken, the legacy file is renamed *once more*, to `<stem>.legacy-<ts>.jsonl`, and becomes a **read-only sidecar** — legacy history that cannot be renamed into place is kept read-only beside the active file; it is searched and loaded, never appended to or rotated. `loadTranscript`, `searchTranscript`, `transcriptCreatedAt` and `countRecentUserMessages` cover sidecars; `append`, `getLastSeq`, rotation and the archive listing cannot see them at all, so `seq` stays unique exactly where uniqueness is required (the active file and its rotation family) and not one byte of history is rewritten to get there. Every file is one atomic `rename`, so every crash point converges when the pass runs again. `migrateSessionKey` settles the old key's filenames before it re-keys (and refuses if that has not settled), carries sidecars across, and applies the same sidecar rule to its own both-exist case, which used to abandon the old transcript on disk. `getLastSeq` takes the maximum over the tail rather than the newest value — and now over the newest *archive* too — so a hand-repaired transcript cannot hand the same seq out twice (rotation reads a re-used seq as "already archived" and drops the record). What the pass could not settle is visible: `store.migrationStatus()` returns `{settled, deferred, ambiguous, sidecars}` and the daemon logs it on start.

  **Readers do not assume a sidecar is older than the file beside it.** It often is not: a re-key sidecars a transcript that ran *in parallel* with the one it lands next to, and a legacy active file an unmigrated process kept appending to is newer than parts of the archive it ends up beside. So with a sidecar in the read set, `searchTranscript` gives up its cross-file early exit (every file read, every record judged on its own) and orders the merged result by the records' own timestamps; `get()` sorts the tail it loaded, so `session.messages` is monotonic and `updatedAt` is the newest record; and `createdAt` takes the minimum across the family rather than the first record it finds. `recall_conversation` with `after` was the live casualty — a lower bound hit on a sidecar ended the scan and abandoned every file behind it, returning nothing while the records sat on disk. `seq` bounds are documented and implemented as **active-family only**: a sidecar is a different seq run that no writer continues, so its numbers are not on the same scale and bounds over them are ignored rather than silently dropping the records. The reader's archive-month set is the **union** of canonical archives and archive sidecars, so an interrupted re-key that moved an archive before its sidecars cannot leave a month that only `transcriptFamilyMoves` can see; `migrationStatus()` reports those as `orphans`. Sidecar records are never rewritten, so a record an interrupted rotation left in both a legacy active file and an archive stays duplicated **on disk**, and a search over that family **reads it twice**: there is no dedupe. Collapsing it cost more than it bought — the only key available is the record itself, and two different senders' identical messages in the same millisecond are indistinguishable from one record read twice, so the filter deleted real messages from search results. And `countRecentUserMessages` reads the active file and the newest sidecar only, because sidecars are not rotated and counting all of them made `/status` grow with every deferred migration the install ever had.

  **The move itself cannot overwrite a live destination.** `existsSync(to)` then `renameSync(from, to)` is a check-then-act, and the window is reachable: `append()` in another process (`tomo config identities` → `pickChatId` → `store.get()`, `tomo lcm search --channel-key`) creates `to` in between, and the rename replaces that brand-new transcript silently. The move is `link` then `unlink` now — atomic, EEXIST when the name is taken (→ sidecar), and a crash between the two leaves both names on one inode, which the re-run recognises by device+inode and finishes with the `unlink` instead of duplicating the file into a sidecar. The same recognition covers the **sidecar** claim, whose name contains the second it was claimed in: a retry looks for an existing `<base>.legacy-*.jsonl` that is the source's own device+inode before claiming a new one, so a crash between the link and the unlink is finished with the `unlink` rather than leaving two readable copies of the same history under two timestamps. Filesystems that cannot hard-link (EPERM / ENOSYS / EXDEV / ENOTSUP) fall back to the old check-then-rename, and which mode is in use is logged once.

  **A malformed ledger row is no longer a malformed ledger.** One bad row in `_legacy_stems.json` deferred *every* key's migration, and the write path then quarantined the file and rebuilt from `{}` — discarding the ownership evidence in every row that was perfectly readable. Rows are parsed one at a time: the good ones are used, the bad ones are moved verbatim into a `_legacy_stems.json.corrupt-<ts>` sibling with the good ones rewritten atomically **on the read path** (so the wedge clears without waiting for a write that would refuse anyway), and only the stems whose own row was unreadable keep deferring. A file that will not parse *at all* is still fail-closed, is now quarantined from the read path too, and is never rebuilt from `{}` on top of the quarantined copy. **And the quarantine holds across processes.** Healing a row leaves a ledger that parses cleanly, so the fact that some stem's owners were unreadable lived only in the healing process's memory — a second process read the healed ledger, saw no partner, and adopted a file whose other owner was named in nothing but the moved rows. The read path now scans for `_legacy_stems.json.corrupt-*` siblings (one `readdir`, nothing opened, when there are none) and treats every stem any copy names — plus, for a copy that cannot be parsed row-wise at all, every stem the healed ledger has no row for — as unknown, deferring those migrations and warning once with the filename. **An operator releases the hold by deleting that `.corrupt-` file** once its rows have been read, or by repairing it into valid JSON. A partner any source can still *see* beats the hold — ambiguity only grows, so a named owner still parks the family rather than merely deferring it.

  **Quarantine is not done until nothing readable is left.** The `.ambiguous-` marker records that the *decision* was taken, not that the work finished — treating it as "done" left the mixed archives of a failed or interrupted pass under readable names with the key marked settled. The family is rescanned on every attempt, and what separates mixed history from what the key has written since is the decision's own timestamp (in the marker's name): a file whose newest record predates it is parked, a file that has grown since is left alone. The active file is parked first and nothing else moves if it cannot be, so a marker always means the active name is the key's own again.

  **A key whose own filename is known to be shared is not served from it.** `get()` ignored a `false` from the migration, so a stable-stem key whose quarantine was deferred (ledger write failed, transcript lock held) kept reading the mixed transcript. It now gets an empty session — and `searchTranscript` returns nothing — until the quarantine lands, warned once. `append()` still works and still writes to that name, which is the one window in which a file known to be shared can grow; it is bounded by the 60s retry, and those records are parked with the rest of the family. A deferral for an *unreadable* source keeps serving the files as before (losing sight of the key's own messages would be the worse failure) and warns. The withheld empty session is cached so an `append()` in that window keeps one session, and that cache is **dropped the moment the key stops being withheld** — it settles, or the probe stops answering `shared` — because another process can park the family and write fresh history between two of this process's attempts, after which the empty cache would keep being served and the next `append()` would hand out `seq` 1 over a file that already had one.

  **The start-up status line says only what it can know.** `settled`/`deferred` count what the process tried and could not finish, and at start it has not tried anything — so they always read clean. Start logs the disk half (`ambiguous`, `sidecars`, `orphans`) and the full `migrationStatus()` is logged once more five minutes in.

  *Removed with the merge it served:* `foldLegacyTranscriptFile`, `shiftJsonlSeq`, the family-wide `seq` offset, the `.premerge-bak` copy and the "folded over an unpaired archive" detector. Every defect the second review round found lived on that path — a second process's `append()` erased between the merge's read and its atomic rename, with no log line; new-stem archives with no legacy counterpart never shifted, so duplicate `seq` values made the next rotation drop months of records; a partially applied merge that invalidated its cache only on full success; offsets recomputed after an interrupted run. A rename has none of those states.

- **The private-memory bar no longer stops at the tool that spells the path.** Two holes, one fence, both closed the same way — by binding the decision to the *turn* rather than to the text of the call.

  **Bash is denied outright on a barred turn** (a group session, or a summoned/mixed-audience turn on the owner's `dm:` session). `bashTouchesMemory` was a token scan over the command the model typed, and an interpreter's argument is a program, not a path: `node -e 'process.stdout.write(require("node:fs").readFileSync("mem"+"ory/pri"+"vate/note.txt","utf8"))'` spells neither `memory` nor `private` in the raw text or in any dequoted token, carries no `$`, no backtick, no glob and no brace — every rule passed it, and the shell read the file anyway. `python -c`, `perl -e`, `osascript -e`, `bash ./helper.sh` and any script written on an earlier turn are the same shape, and they are not a list that can be finished. The guard's own header already conceded the point ("on a barred turn Bash is a convenience the model does not need") while the code kept trying to filter; the tool now goes instead of the spelling. The denial says why the shell is gone *and* why this turn is barred, so the model explains rather than retries. `bashTouchesMemory` stays as the narrower predicate and its tests stand; it is simply no longer what decides. Subagent calls travel the same PreToolUse hook (the SDK propagates a session's hooks into Agent-tool children), which matters because `agentProfiles` fails open for an unprofiled type.

  **A `MEDIA:` attachment is a read, and is now judged as one.** The model could write `MEDIA:"memory/private/x.png"` in a reply block, or hand the same string to `send_message` in `direct` mode, and the harness opened the file and put its contents in the group — with only an `existsSync` between it and the wire. No tool call happens on that path, so nothing the PreToolUse guard fenced applied to it: a turn that could not `Read` the file could still send it. `send_message` now has its own arm in `isPrivateMemoryAccess` (the tool call is refused before it runs, whatever the mode), and reply delivery drops a private `MEDIA:` path before the send — treated exactly like a file that is not there, so the caption still delivers as text and the transcript records the picture as unsent rather than claiming a delivery. Both sides ask one predicate (`landsInPrivate`, shared with the tool guard), so a symlink parked under a public directory is caught by the path it really opens, and one function (`privateMemoryBarFor`) decides whether the turn is barred, so the inbound and outbound halves cannot drift apart. The group system prompt says both things out loud.

- **`every 0s` is refused instead of pinning the scheduler to one job.** `parseScheduleString` turned it into `{ kind: "every", everyMs: 0 }`, `computeNextRun` answered `fromMs + 0` — a perfectly good timestamp, so `unschedulableReason` saw nothing wrong and `schedule_create` / `tomo cron add` accepted it. `markRun` then set `nextRunAt` to the moment the run finished, and the job came back due on every 30s poll, forever. Recurring intervals are now bounded on both ends: at least `MIN_EVERY_MS` (30s, pinned by a test to the scheduler's poll interval — a job cannot fire more often than the scan that dispatches it) and at most 366 days, which also catches `every 99999999999999d`, an interval that multiplies past `Number.MAX_SAFE_INTEGER` and would have been stored `enabled: true` with a `nextRunAt` no clock ever reaches. The bound is enforced at three depths: `unschedulableReason` (so both creation surfaces refuse it in one line, naming the actual defect), `CronStore.add` (so a caller that skipped validation cannot write one), and `load()`, where a job already on disk with a bad interval — hand-edited, or written by an older build — is held disabled with one warning per process rather than either firing or taking the whole schedule down as a read error. The file itself is left as the user wrote it; this refuses to run a job, it does not silently rewrite one.

- **The watch server no longer throws on valid JSON that is not a frame.** `handleClientLine` cast the parse result straight to `ClientFrame`, so `null`, `42`, `[1]` and `"str"` got past the try/catch and died on `frame.kind` — a TypeError raised from a fire-and-forget dispatch, i.e. an unhandled rejection caught by the global process handler. The decoded value is now shape-checked (a plain object with a string `kind`) and anything else gets the existing `invalid frame` refusal, with a `.catch` on the dispatch as a second floor.

- **A test fixture no longer depends on the host time zone.** `resolveTimeRange` reads a zoneless boundary as host-local by design, and one JSONL test paired zoneless boundaries with UTC event stamps — so the suite failed east of UTC (`TZ=Asia/Shanghai`) for reasons unrelated to what the test asserts. Boundaries are explicit UTC now.

- **Vulnerable transitive dependencies refreshed.** Lockfile-only bump of `fast-uri` 3.1.5 → 3.1.7 (GHSA-f65p-4m7j-42xc, high) and `qs` 6.15.3 → 6.16.0 (GHSA-4mjr-xmp4-gh2g, moderate), both reached through `@anthropic-ai/claude-agent-sdk` → `@modelcontextprotocol/sdk`. Both fit the existing caret ranges; no direct dependency changed and `npm audit` is clean.
...
- **Brace expansion no longer walks the private-memory Bash guard.** `GLOB_META` was `/[*?[]/`, so `bashTouchesMemory` classified `{m,}emory/{p,}rivate/x.md` as an ordinary literal token: brace expansion is not globbing, it fires whether or not anything matches on disk, and the command spells neither `memory` nor `private` at any word border. Run against the live guard on a barred turn it returned the file. `{` is now a metacharacter like the rest — its segment yields an empty literal prefix, which is a prefix of everything, so a brace in a path-ish token is denied exactly as a bare `*` already was. Two neighbours of the same class went with it, because all three are the shape the module's own header already names as unparseable — *the word the shell ends up with is not the word the hook was handed*. `$` and backticks are refused outright: command substitution runs arbitrary code to produce a path (`cat $(echo memory/private/x)`, the header's own example), and parameter expansion assembles one out of pieces no rule can see (`d=memory e=private; cat $d/$e/x`). And the `memory`/`private` segment rule is now re-applied to each token with its quotes stripped, so adjacent-string concatenation (`cat "mem""ory"/private/x.md`, `cat me''mory/...`) — one word to the shell, two quoted runs to a regex reading the raw command — puts the name back at a border. `$HOME`-style conveniences are the accepted cost, the same call shapes 3-6 already make: on a barred turn Bash is a convenience the model does not need, and Read on a named public file is the intended route.

- **A Glob or Grep rooted on a symlink to `memory/` no longer walks the private tree.** The guard real-resolved one side only. `landsInPrivate` follows symlinks, but it asks whether the path lands at-or-inside `private/` — a link pointing at `memory/` ITSELF (`<ws>/notes -> <ws>/memory`, a link the agent is free to create) is not, so it passed — and every containment test after it ran on the LEXICAL root, where `relative("<ws>/notes", "<ws>/memory/private")` is `../memory/private`, i.e. "private/ is not reachable from this root". `Grep({ path: "notes", pattern: "..." })` recursed through the link and read every DM-only record on a group turn. Both search arms now judge the root in two frames — as spelled, and as it really resolves — and the memory and private dirs are re-resolved ALONGSIDE the real root rather than mixed with it, since the private dir may itself sit under a symlinked prefix (`/tmp` -> `/private/tmp` on macOS) and a mixed comparison fails open the other way. The realpath cache that already existed for `private/` is now shared by both dirs, so the extra resolution costs nothing after the first call.

- **The `.claude/skills/` re-allow no longer hands back the paths the SDK protected.** `skillsCanUseTool` is the callback the SDK falls through to for writes into `.claude/`, `.git/` and friends under `bypassPermissions`, and it re-approved on `filePath.startsWith(SKILLS_DIR)` and `command.includes(SKILLS_DIR)`. Both over-match, and this is an ALLOW predicate, so over-matching *grants*: `<ws>/.claude/skills/../settings.local.json` starts with the prefix and lands in `.claude/`, and any Bash command that mentioned the skills path anywhere — in a trailing comment, in an `echo`, on the far side of the `rm` that did the damage — was approved whole. Paths now go through the same `realResolve` the private-memory guard uses (it collapses `..`, follows symlinks, and tolerates a file that does not exist yet, which is the normal case for a write) and then an exact containment test. Bash needs all three of: at least one token really inside the skills dir, no token anywhere else in a protected tree, and no `$`, backtick or `~` — each of those produces a word the scan cannot see, and the last is how `~/.claude/settings.json` hides from a token walk rooted at the workspace. Containment here is CASE-SENSITIVE, unlike the guard's: a failed comparison denies, so exactness is the conservative direction on this side, and `.claude/SKILLS/` is a different directory on a case-sensitive volume. Two comments citing a "PR #305" helper that never landed are corrected — `realResolve` is now genuinely the shared helper they described.

- **One malformed request to the OAuth callback port no longer kills the daemon.** The loopback listener built its URL as `new URL(req.url, \`http://${req.headers.host}\`)`. That header is whatever the client sent, and `Invalid URL` is thrown for an empty, spaced or bracketed value — `Host:` with nothing after it is the cheapest of them, and the `?? "127.0.0.1"` fallback never covered it because `""` is not nullish. The throw happened inside the `'request'` listener, which makes it an uncaughtException: `process-handlers.ts` logs it and exits 1, so anything able to open a socket to a transient callback port could stop Tomo — including an ordinary port scanner, since the listener answers on every path. The base is now pinned to `http://127.0.0.1` (the listener binds loopback and routes on the path alone, so the authority was only ever bookkeeping), the target must be origin-form — a `//`-leading target is protocol-relative and would move the authority onto whatever followed while keeping a pathname that routes as a real callback — and anything unparseable gets a 400. The whole handler is additionally wrapped, because the invariant that matters is not "this line does not throw" but "nothing in a request listener throws".

- **A refresh token the issuer has revoked is now reported, and stops being posted every minute.** The background sweep runs on a 60s timer and a rejected exchange left no trace whatsoever: `refreshServerToken` recorded the message on `serverFailures` — read only by `/mcp` status, which nobody looks at until something is already broken — and returned. So a refresh token that had been revoked, or rotated away by a login on another machine, was posted to the issuer 1,440 times a day, indefinitely, with nothing in the log to say so. Each rejection now logs a `failed` outcome naming the server, the store key, the consecutive-failure count and the retry delay, and rests that key for an exponentially growing interval from one sweep up to an hour — roughly thirty attempts a day instead of 1,440, while still recovering on its own if the issuer was merely having a bad minute. Only the SWEEP rests: a 401 from a live server and a hand-run `/mcp login` are things someone is waiting on, and they still try immediately. Any successful write to the key clears the rest period, so an interactive login is not made to wait out a backoff that was about the credential it just replaced. A rejection is counted once per exchange, not once per server sharing the key.

- **A refresh response without `expires_in` no longer erases the stored expiry.** `normalizeToken` always carries an `expiresAt` property, so `{ ...existing, ...normalizeToken(...) }` spread `undefined` over a perfectly good stored value whenever the issuer omitted the field — which is legal, and which some issuers do on refresh. `isExpiring` reads `undefined` as "never expires", so the sweep stopped looking at the key entirely and the access token died silently in an hour, every call 401ing until the backstop noticed. The expiry is now carried forward: a stored value still in the future is kept as the best evidence available, one already in the past is replaced with a conservative one-hour default (keeping a stale reading would leave the record permanently inside the refresh skew window and re-refreshed on every sweep), and `undefined` stays `undefined` — the issuer has never said, and inventing an expiry would start refreshing a token that was working.

- **launchd no longer SIGKILLs the daemon in the middle of its shutdown.** The LaunchAgent plist set `KeepAlive`, `RunAtLoad` and a `ThrottleInterval` but no `ExitTimeOut`, so launchd used its 20-second default: SIGTERM, twenty seconds, SIGKILL. Tomo's graceful shutdown is bounded at `DAEMON_STOP_TIMEOUT_MS` — 60s, and around 33s in practice once the channel drains, the final registry save and the transcript flush are counted, which is why `tomo stop` waits that long. Every shutdown launchd started therefore cut the daemon off mid-flush: `launchctl bootout`, a restart through launchd, a logout, a reboot. `ExitTimeOut` is now emitted and derived from `DAEMON_STOP_TIMEOUT_MS` rather than written out as a number, so a shutdown launchd starts gets exactly as long as one a human starts and the two cannot drift; a test asserts both the derivation and the rendered plist.

- **`tomo backup` no longer reports a complete backup with no conversation history in it.** `runtime-paths.ts` lets `SESSIONS_DIR` move the session registry and every transcript out of `~/.tomo/data` — onto a bigger disk, out of a synced folder — and `backup create` copied `TOMO_HOME/data`, the workspace, `config.json` and the SDK session files, full stop. On an overridden install it therefore printed "Backup complete" over an archive holding no registry and no transcripts, and the failure only surfaced on the restore that came back with no history at all: silent in both directions, which is the worst shape a backup bug can have. A `sessions/` leg now sits alongside `sdk-sessions/`, added only when the directory is genuinely outside `data/` so a default install does not copy the same tree twice. Restore knows about it too — as a staged leg, in the `RESTORE_LEGS` symlink/kind check, and in the interrupted-restore leftover sweep — and puts the transcripts wherever THIS install keeps them, which is the only place the daemon will look for them.

- **A registry write that fails during expired-session cleanup no longer stops the daemon starting.** `cleanupExpired` runs from the `SessionStore` constructor and its own header says it must not throw — it already refuses to run at all on a registry it could not read, precisely because it unlinks SDK JSONL files irreversibly. The save at the end of it was a bare `saveRegistry()`, which throws on any write failure: ENOSPC, EACCES, a read-only volume. That propagates out of `new SessionStore()`, so the daemon does not start and cannot receive a message on any channel, over housekeeping that was never urgent — and it happens AFTER the unlinks, so the boot that failed had already deleted the files. It now goes through the existing `saveRegistryBestEffort`, which logs once per streak and keeps the change in memory; the entries left on disk name transcripts that no longer exist, which is harmless and is swept again on the next start once the write works.

- **Person records are written atomically.** `savePersonRecord` used a plain `writeFileSync`, which truncates first and fills after — and it runs on the MESSAGE path, from `upsert_person` during a turn and from the handle auto-binding that fires the first time a group sender's display name matches an unbound record. So it races every reader of the same file: `loadDir` on the next inbound message, another upsert, a restart. A reader landing in that window gets a file with no parseable frontmatter, `parsePersonFile` returns undefined, and the person stops being recognised — silently, because the loader treated an unparseable record as nothing at all; a crash in the same window makes it permanent. The write is now `writeFileAtomicSync` (temp file plus rename, corpse cleaned up on failure), and a `.md` file in a people directory that yields no usable `name:` is now named in a warning instead of being dropped without a word, so a torn or hand-broken record can be repaired rather than guessed at.

- **A continuity script that leaves a background process no longer stops the heartbeat forever.** `runContinuityScript` resolved on the child's `'close'` event, which fires only once EVERY holder of its stdio has let go — and a backgrounded grandchild inherits those pipes. So `status.sh` ending in `sleep 30 &` delayed the heartbeat by thirty seconds, and one that started a daemon without redirecting its output stopped the heartbeat permanently: not a slow heartbeat, no heartbeat at all, for the life of the process. The timeout was no help, because `child.kill()` signals the direct child only and the script had already exited. The child is now spawned `detached`, so it leads its own process group and the timeout can signal the group — SIGTERM, then SIGKILL a second later — reaching everything the script started. The result is taken from `'exit'`, where the script's own status is known, with a one-second drain afterwards for output still in flight (which is all `'close'` was ever wanted for) and then an answer whether or not anything else is still holding the pipes. The read ends are destroyed and the child unref'd on the way out, so a surviving grandchild cannot pin the event loop to a run nobody is waiting on any more.

- **A `<tomo-event …>` line the model writes is marked, not cut — and a fenced one is left alone.** `<tomo-event` was listed in the scaffold filter's `ENVELOPE_LINE_RES`, which truncates the reply from the first matching line onward and knows nothing about code fences. It is also one of `inbound-markers.ts`'s `MARKER_SHAPES`, where the policy CLAUDE.md and this changelog both state is MARK, DON'T TRUNCATE, and where lines inside a ``` or ~~~ fence are skipped. Both ran on the same text, in that order, inside `renderBlock` — so the marker guard's tomo-event arm was dead code (the shape had already been removed by the time it looked), and a pasted log or transcript excerpt inside a fence, which is exactly what the fence rule exists to protect, was cut at the fenced line. `tomo-event` is dropped from the truncating filter; the fence-aware owner keeps it. `<system-reminder>`, `<task-notification>`, `<task-id>` and `<function_results>` have no other owner and still truncate. A test drives all three cases through `renderBlock`, which is the only place the handoff between the two passes is observable.

- **`tomo migrate openclaw` imports into the workspace the daemon actually reads.** The command re-derived `join(homedir(), ".tomo", "workspace", "memory")` instead of going through `runtime-paths.ts`, so it ignored `TOMO_WORKSPACE` — the override every other reader of memory honours. On a relocated workspace the imported conversation and the `MEMORY.md` index line were written into a directory nothing loads, and the command then printed that path and said "Tomo will see this conversation in its memory on the next message", which was simply false. It now uses `MEMORY_DIR`, the same constant the system-prompt builder and the private-memory guard use.

- **The private-memory Bash guard no longer misses a backslash-quoted path.** `bashTokens` dequoted `"`, `'` and backticks and left the fourth quoting operator in the token. `\` quotes the single character after it rather than a run, so `cat mem\ory/priv\ate/x.md` and `cd mem\ory && cat priv\ate/x.md` are ordinary reads of `memory/private/x.md` — real bash prints the contents — while the `memory`/`private` segment rule was handed `mem\ory` and found neither name at a word border. Both shapes were ALLOWED on a barred turn, which is the same walk-through the quoted-string concatenation case above closed, one operator later. The backslash is now stripped with the quotes, so every token rule runs on the word the shell will build.

- **The `.claude/skills/` re-allow no longer resolves a `cd`-ed path against the workspace.** `bashStaysInSkills` judges every relative token against `config.workspaceDir`, a fixed root that has nothing to do with the directory the command will run in — so `cd /ws/.claude/skills && rm -rf ../settings.local.json` was ALLOWED: the first token lands in the skills carve-out, and `../settings.local.json` resolved against `/ws` is `/settings.local.json`, which sits in no protected tree at all. Run for real it deletes the permissions file the SDK falls through to this callback to protect. `cd`, `pushd` and `popd` now refuse the command outright, and so does a `..` path SEGMENT in any token — the same call this predicate already makes for `$`, backticks and `~`, and for the same reason: the word it is being handed is not the path that will be touched. `a..b.md` is an ordinary name and still passes.

- **A `--flag=path` no longer hides its destination from the `.claude/skills/` re-allow.** `bashStaysInSkills` skipped any token starting with `-` as "a flag names no path", which is true of `-r` and false of the form every long option uses to carry an argument: `--flag=value` is one word, and the whole of it was skipped. So `tar -xf /ws/.claude/skills/x.tar --directory=/ws/.claude` and `cp /ws/.claude/skills/a.md --target-directory=/ws/.claude/agents` were auto-approved on the strength of their SOURCE path, with the destination — inside the tree the SDK protects — never looked at. Words are now split at their first `=` and the right-hand side judged as a path like any other; a non-flag word is offered whole and split, so a `NAME=path` assignment is judged on its value too. The two-token form (`-C ../.claude`) never needed this — the value is its own word and the loop has always seen it.

- **A restore no longer drops the `sessions/` leg onto an install without `SESSIONS_DIR`.** The leg's destination was `externalSessionsDir()`, which answers null when this install keeps its transcripts inside `~/.tomo/data` — and a null destination dropped the leg without a word. So restoring a backup taken WITH the override onto a machine without it (a rebuild, a second machine, an edit to the env) printed "Restore complete." over a `sessions/` directory nobody read, and the operator's whole conversation history was gone with nothing in the output naming it. The same silent-in-both-directions shape the backup half of this bug had. The destination is now `config.sessionsDir` — where this install actually reads them from, `<TOMO_HOME>/data/sessions` by default — so the transcripts follow the current setting in both directions. That destination sits INSIDE the `data/` leg, and `restoreLegsStaged` stages each leg beside its own destination before swapping any of them, so a nested leg would lose its staging directory when `data/` swapped: it therefore runs as a second staged transaction once `data/` is in place, and a failure there says so and names both the backup's copy and where this install reads from rather than claiming completion.

- **A carried-forward OAuth expiry inside the refresh skew no longer causes a refresh loop.** `carryForwardExpiry` kept a stored expiry on the test `previous > now`, but the cutoff that reads the value back is `isExpiring`'s `now + 5 minutes`. An expiry a minute out is in the future AND already inside the skew window, so a refresh response with no `expires_in` stored a record the very next 60s sweep judged expiring — refreshed it, carried the same near-expiry reading forward, and did it again, for as many sweeps as the window holds: roughly five pointless token-endpoint exchanges per refresh, each one a chance to trip a rate limit. The test is now the same one the sweep uses, so anything it would call expiring is replaced with the conservative one-hour default rather than carried.

- **A malformed person file is warned about once, not once per message.** `loadDir` logged a warning for every unreadable, oversized or frontmatter-less `.md` in the people directory on every `loadPeople` — and `loadPeople` runs on the inbound message path, several times per message before the request-scoped snapshot and once per message after it. One hand-edited or half-written record therefore wrote the same line at the rate the chat was busy, forever, drowning the log the warning exists to be visible in. Each path is now reported once per process per reason, cleared by a later load that parses it — so a record repaired and broken again is reported again rather than ignored for the life of the daemon, and a second broken file is still named.
