...
/**
 * Peel a trailing bare-NO_REPLY block off a joined multi-block response.
 *
 ...
 * Only the last line is inspected: blocks are pre-trimmed before joining, so
 * a NO_REPLY block never has interior newlines, and this can't
 * false-positive on an earlier block that merely mentions NO_REPLY inline.
 */
export function stripTrailingNoReply(response: string): { visible: string; hadTrailingNoReply: boolean } {
  const lines = response.split("\n");
  if (!isSilentReply(lines[lines.length - 1] ?? "")) {
    return { visible: response, hadTrailingNoReply: false };
  }
  lines.pop();
  while (lines.length > 0 && lines[lines.length - 1].trim() === "") lines.pop();
  return { visible: lines.join("\n").trim(), hadTrailingNoReply: true };
}
