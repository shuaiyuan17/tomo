...
| `showThinking` | boolean | Optional. Default `false` (hidden). The model sometimes leaks its internal reasoning into a reply as a `思考:` preamble with the real answer glued onto the end; the outbound scaffold filter removes the preamble and keeps the answer. Set `true` or `TOMO_SHOW_THINKING=true` to have the reasoning delivered as well. Conversation-envelope scaffold and tool-call debris (a bare trailing `count` line) are stripped either way — this switch governs only the thinking preamble. |
