...
| `showThinking` | boolean | Optional. Default `false`. Deliver the model's `thinking` content blocks to the chat alongside its reply, each prefixed with `💭`. Purely a content-block-type decision — outbound text is never pattern-matched. Also settable with `TOMO_SHOW_THINKING`. **Read at session start only.** The flag also sets the SDK's thinking `display` (`omitted` when off, `summarized` when on), and that is fixed when the session spawns — changing it mid-session takes effect on the next session, so `tomo restart` (or `/reset` for one session) to apply it now. |
