...
| `showThinking` | boolean | Optional. Default `false`. Deliver the model's `thinking` content blocks to the chat alongside its reply, each prefixed with `💭`. Purely a content-block-type decision — outbound text is never pattern-matched. Also settable with `TOMO_SHOW_THINKING`. |
