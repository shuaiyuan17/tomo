...
- **`schedule_enable` MCP tool.** Enables (or disables) a scheduled task by id without deleting and re-creating it. The agent could previously only create, list and remove jobs, so a job the daemon disabled — a one-shot whose run was interrupted by a restart, a job that hit the resume cap — had no path back except re-creating it from scratch, losing its id and history. Enabling clears the interrupted state as well as flipping the flag: it acknowledges the stale run token, so the next daemon start does not settle the job as interrupted and disable it all over again. The tool description says what an interrupted job means and that its work may already be done, because "just re-enable it" is the wrong reflex for a task that may already have sent, ordered, or posted.

- **`schedule_enable` MCP tool.** Enables (or disables) a scheduled task by id without deleting and re-creating it. The agent could previously only create, list and remove jobs, so a job the daemon disabled — a one-shot whose run was interrupted by a restart, a job that hit the resume cap — had no path back except re-creating it from scratch, losing its id and history. Enabling clears the interrupted state as well as flipping the flag: it acknowledges the stale run token, so the next daemon start does not settle the job as interrupted and disable it all over again. The tool description says what an interrupted job means and that its work may already be done, because "just re-enable it" is the wrong reflex for a task that may already have sent, ordered, or posted. The tool is **scoped to the calling session**: it only acts on jobs whose `sessionKey` is the caller's, so a group chat cannot restart a task that fires into the owner's DM, and the refusal does not name the owning session. `schedule_list` / `schedule_create` / `schedule_remove` remain unscoped — that is issue #312, deliberately neither widened nor narrowed here.
