...
- **Two people no longer share one transcript file.** `SessionStore` named a transcript by replacing every character outside `[A-Za-z0-9_-]` with `_`, which is many-to-one: `imessage:any;-;alex.smith@example.com` and `imessage:any;-;alex_smith@example.com` are two different contacts that both resolved to `imessage_any_-_alex_smith_example_com.jsonl`, and with them the whole `_archive_<name>_<YYYY-MM>.jsonl` set. Everything downstream followed the filename rather than the key — `get()` loaded one person's tail into the other's session, `append()` interleaved them, `searchTranscript` and `recall_conversation` returned one contact's messages when asked about the other, and rotation rewrote the shared file for whichever key happened to trigger it. Any two keys differing only in a character the replacement flattens (`.` vs `_`, `@` vs `+`, a Unicode name against its punctuation twin) hit it.
  ...
  The migration never throws on the message path — a lock held by another process, an EACCES, a rename that loses a race all degrade to "don't migrate", logged once. Crucially it is also never *recorded* as done in that case: marking the key first meant one transcript-lock timeout was permanent. The message that triggered it went on to `append()` under the new stem, both files then existed, and the both-exist branch left the legacy file alone forever — so every message older than the failed migration was gone from history, search, `createdAt` and seq allocation while sitting intact on disk. Now an incomplete attempt is retried (throttled, so a held lock does not cost a second per message), and the both-exist case is recovered rather than refused: under the lock the legacy file's records are folded onto the head of the migrated file, with the newer file's `seq` values shifted up past them so the two runs form one increasing sequence, the replacement published by atomic rename with a `.premerge-bak` alongside, and the legacy file removed only afterwards. Re-running is safe — a crash between the two is detected by the legacy bytes already sitting at the head of the target. `getLastSeq` now takes the maximum over the tail rather than the newest value, so a folded transcript cannot hand the same seq out twice (rotation reads a re-used seq as "already archived" and drops the record). `migrateSessionKey` still settles the old key's filenames before it re-keys the entry, so identity unification carries the transcript and its archives across.
